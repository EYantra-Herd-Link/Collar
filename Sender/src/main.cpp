...
#define JERK_WALKING  5.0f
#define JERK_GRAZING  1.5f
#define JERK_MIN      0.3f
...
void sensorTask(void *pvParameters) {
  for (;;) {
    ...
    if (ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(SENSOR_POLL_RATE_MS)) > 0) {
      ...
      Serial.println("WAKEUP: Timer Poll");
    }
    ...
    if (rec.state == RESTING && rec.audioMagnitude > RUMINATION_AUDIO_THRESHOLD) {
    ...
    if (xQueueReceive(healthDataQueue, &rec, pdMS_TO_TICKS(5000)) == pdPASS) {
      ...
      if (WiFi.status() == WL_CONNECTED) {
        ...
        snprintf(payload, sizeof(payload),
          ...
          (int)rec.state,
          rec.is_interrupt ? 1 : 0,
          (rec.activityJerk < JERK_MIN && rec.activityJerk >= 0.0f) ? 1 : 0,
          (int)WiFi.RSSI(),
          af_buf
        );
